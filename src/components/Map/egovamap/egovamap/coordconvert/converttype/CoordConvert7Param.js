...
var CoordConvert7Param = {
	val1: 0,
	val2: 0,
	val3: 0,
	//7参数
	r: [0, 0, 0, 0, 0, 0, 0], //[X平移,Y平移,Z平移,X旋转,Y旋转,Z旋转,以及尺度比参数]
	//4参数
	DX: 0,
	DY: 0,
	T: 0,
	...
	wgsll2beijing54sevenparam: function (x, y) {
		var params = CoordConvConfig.coordConvert4Params;
		return this.coordConvert(0, params, x, y);
	},

	wgsll2xian80sevenparam: function (x, y) {
		var params = CoordConvConfig.coordConvert4Params;
		return this.coordConvert7Params(1, params, x, y);
	},
	/**
	 * 7参数转换初始化
	 ...
	 */
	coordConvert7Params: function (sysID, params, x, y) {
		...
		if (ps.length >= 1) {
			...
			this.r = rr;
			console.log("sysID=" + sysID + ",x0=" + x0 + ",y0=" + y0 + ",z0=" + z0 + ",rx0=" + rx0 + ",ry0=" + ry0 + ",rz0=" + rz0 + ",m=" + m);
		} else {
		...
		if (ps.length >= 2) {
			...
			if (pams.length == 1) {
				...
				if (pams.length >= 6) {
					L0 = parseFloat(pams[5].trim()) / 180 * Math.PI;
				}
			}
	 ...
	 */
	setCoordType: function (coordType) {
		switch (coordType) {
	...
	gauss2Earth: function (dL0) {
		...
		var l = (1 - (b3 - b5 * Z2) * Z2) * Z * CoordConst.P_2;
